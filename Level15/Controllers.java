...
public class Controllers {
    ...
    public static void main(String[] args) throws IOException {
        ...
        while (q-- > 0) {
            ...
            if (a == b) {
                // If buttons are same value but counts differ, net cannot be zero
                System.out.println("NO");
                continue;
            }

            // long g = gcd(a, b);
            // long l = (a * b) / g; // fits in long (64-bit) -> lcm = a * b / gcd
            long l = lcm(a, b);
            long x = l / a;
            long y = l / b;

            long diffCounts = Math.abs(c1 - c2);
            long diffXY = Math.abs(x - y);

            if (diffCounts % diffXY == 0) {
                long p = diffCounts / diffXY;
                if ((x + y) * p > n) {
                    System.out.println("NO");
                } else {
                    System.out.println("YES");
