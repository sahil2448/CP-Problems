...
public class KAmazingNumbers {
    ...
    static void solve() throws IOException {
        ...
        for (int val : values) {
            List<Integer> ls = posMap.get(val);
            if (ls == null || ls.isEmpty()) {
                continue;
            }
            int mx = 0;
            mx = Math.max(mx, ls.get(0));
            for (int i = 1; i < ls.size(); i++) {
                mx = Math.max(mx, ls.get(i) - ls.get(i - 1) - 1);
            }
            mx = Math.max(mx, (n - 1) - ls.get(ls.size() - 1));
