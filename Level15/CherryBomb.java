...
public class CherryBomb {
    ...
    static void solve() throws IOException {
        ...
        long k = sc.nextLong();
        long[] a = new long[n];
        long minA = Long.MAX_VALUE, maxA = Long.MIN_VALUE;
        ...
        if (knownIdx == -1) {
            long count = (minA + k) - maxA + 1;
            System.out.println(Math.max(0, count));
        } else {
            long X = a[knownIdx] + b[knownIdx];
            boolean ok = true;
            for (int i = 0; i < n; i++) {
                long requiredB = X - a[i];
                if ((b[i] != -1 && b[i] != requiredB) || (requiredB < 0 || requiredB > k)) {
                    ok = false;
                    break;
                }
            }
