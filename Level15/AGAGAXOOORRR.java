...
public class AGAGAXOOORRR {
    ...
    static void solve() throws IOException {
        ...
        // Case 1: Total XOR is 0, we can always make 2 equal elements
        if (totalXOR == 0) {
            System.out.println("YES");
            return;
        }

        // Case 2: Try to find 3 segments that each XOR to totalXOR
        int count = 0;
        long currentXOR = 0;
        for (int i = 0; i < n; i++) {
            currentXOR ^= a[i];
        ...
        // If we found 3 or more segments, we can group them into exactly 3
        if (count >= 3) {
            System.out.println("YES");
        } else {
