...
public class C {
    ...
    static void solve() throws IOException {
        ...
        long[] suffixSums = new long[n + 1];
        suffixSums[n] = 0;
        for (int i = n - 1; i >= 0; i--) {
            suffixSums[i] = suffixSums[i + 1] + a[i];
        }
        ...
        for (int i = 0; i < n; i++) {
            ...
            if (i == 0) {
                currentX = -suffixSums[1];
            } else {

                currentX = a[0] + curr - suffixSums[i + 1];
            }
    ...
    static int lastLessThanOrElistual(int[] arr, int target) {
        int low = 0, high = arr.length - 1;
        int ans = -1;
        while (low <= high) {
            int mid = (low + high) >>> 1;
