...
public class B {
    ...
    static void solve() throws IOException {
        ...
        long diffSum = 0;
        for (int i = 1; i < n; i++) {
            diffSum += Math.abs(a[i] - a[i - 1]);
        }

        long ans = diffSum;

        ans = Math.min(ans, diffSum - Math.abs(a[1] - a[0]));
        ans = Math.min(ans, diffSum - Math.abs(a[n - 1] - a[n - 2]));

        for (int k = 1; k <= n - 2; k++) {
            long del = Math.abs(a[k] - a[k - 1]) + Math.abs(a[k + 1] - a[k]);
            long add = Math.abs(a[k + 1] - a[k - 1]);
            long min = diffSum - del + add;
            if (min < ans) {
                ans = min;
            }
        }
