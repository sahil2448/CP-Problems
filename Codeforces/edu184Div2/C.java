...
public class C {
    ...
    static void solve() throws IOException {
        ...
        long originalTotalSum = p[n];
        long maxTotalSum = originalTotalSum;

        long minG = Long.MAX_VALUE;

        for (int i = 1; i <= n; i++) {
            long l = i;
            long currentG = (l * l) - l - p[(int) l - 1];
            ...
            long r = i;
            long currentF = (r * r) + r - p[(int) r];

            long currentGain = currentF - minG;

            if (originalTotalSum + currentGain > maxTotalSum) {
                maxTotalSum = originalTotalSum + currentGain;
            }
        }

        out.println(maxTotalSum);
    }
